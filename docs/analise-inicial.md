...
## Pagamentos integrados no PDV — Mercado Pago (2026-09-24)

**Pesquisa (documentação oficial, não memória).** Mercado Pago: Orders presencial (QR `type: "qr"`, Point `type: "point"`), cancel/refund, idempotência, assinatura HMAC do webhook. O Pix "online" exige `payer.email`: descartado para o balcão; o QR presencial não pede dados do comprador. PagBank: homologação obrigatória (até 4 dias úteis), `customer.tax_id` obrigatório, maquininha só via SDK Android SmartPOS (parceria comercial). Asaas: cobrança exige cliente com `cpfCnpj`. Efí: API Pix padrão BACEN com mTLS/certificado e webhook que exige mTLS no nosso servidor (Traefik/Easypanel). PicPay: credenciais do Painel Lojista e certificação PCI DSS exigida — excluído pela condição do usuário ("se não exigir homologação").

**Decisões.** Venda `PENDING_PAYMENT` com estoque reservado em vez de "cobrar antes, criar venda depois" (evita dinheiro recebido sem venda quando o estoque acaba no meio). Confirmação só por consulta autenticada (webhook e tela e worker são gatilhos). Verificação de valor e `external_reference` antes de concluir. Estorno total no provedor antes de cancelar venda paga. Resolução manual de `action_required` (documentado pelo Mercado Pago como estado que não muda sozinho). Fingerprint de idempotência continua em hash (o comando pode conter senha de supervisor).
