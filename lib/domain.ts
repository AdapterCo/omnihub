...
export type Sale={id:string;storeId:string;storeName:string;storeCnpj:string;cashId:string;userId:string;operator:string;createdAt:number;items:{productId:string;name:string;sku:string;qty:number;price:number;discount:number;returnedQty:number}[];total:number;discount:number;discountReason:string;discountAuthorizedBy:string;returnedTotal:number;returns:SaleReturn[];payment:string;customer:string;document:string;status:string;fiscalStatus:'pending';printCount:number};
...
export type { DiscountLimitView } from './sales/discount.ts';
export type { PaymentConfigSummary, ChargeView } from './payments/service.ts';
import type { PaymentConfigSummary, ChargeView } from './payments/service.ts';
import type { DiscountLimitView } from './sales/discount.ts';
...
export type Snapshot={account:{name:string;subscription:Entitlement};actor:Actor;state:{stores:StoreRecord[];products:Product[];stock:Record<string,Record<string,number>>;transfers:Transfer[];cash:Cash[];sales:Sale[];audit:Audit[];users?:TenantUser[];customers?:CustomerRecord[];suppliers?:SupplierRecord[];fiscalConfigs?:Record<string,FiscalStoreConfig>;fiscalDocuments?:Record<string,FiscalDocumentSummary>;fiscalInutilizations?:Record<string,FiscalInutilizationSummary[]>;nfceConfigs?:Record<string,NFCeStoreConfig>;myDiscountLimitBp?:number;discountLimits?:DiscountLimitView[];paymentConfigs?:Record<string,PaymentConfigSummary>;openCharges?:ChargeView[]};revision:number};
